...
import type { PathContext } from './buildPath.js'

const main: PathContext = { section: 'main' }
const suppl: PathContext = { section: 'suppl', amendLawNum: '平成一五年五月三〇日法律第六一号' }
const supplOriginal: PathContext = { section: 'suppl' }
...
describe('buildPath', () => {
  describe('セクションルート', () => {
    ...
    it('SupplProvision with AmendLawNum', () => {
      expect(buildPath('SupplProvision', null, 0, '', suppl)).toBe(
        'suppl/平成一五年五月三〇日法律第六一号',
      )
    })
    ...
    it('Appdx系 → appdx/{orderIndex}', () => {
      expect(buildPath('AppdxTable', null, 2, '', { section: 'appdx', appdxIndex: 2 })).toBe(
        'appdx/2',
      )
    })
  })
  ...
  describe('Subitem', () => {
    ...
    it('Subitem2 → s2-{num}', () => {
      expect(buildPath('Subitem2', '1', 0, 'main/a003/p1/i2/s1-3', main)).toBe(
        'main/a003/p1/i2/s1-3/s2-1',
      )
    })
  })
  ...
  describe('附則内のArticle', () => {
    it('suppl内のArticle', () => {
      expect(buildPath('Article', '1', 0, 'suppl/__original__', supplOriginal)).toBe(
        'suppl/__original__/a001',
      )
    })
  })
})
