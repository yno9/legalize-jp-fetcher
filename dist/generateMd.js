...
async function main() {
    ...
    if (existsSync(jpDir)) {
        for (const file of readdirSync(jpDir)) {
            if (file.endsWith('.md'))
                allMdFiles.push(path.join('jp', file));
        }
    }
    ...
    for (let i = 0; i < laws.length; i++) {
        ...
        if (law.current) {
            const fulltextPath = join(FULLTEXT_DIR, `${law.current.revisionId}.json`);
            if (existsSync(fulltextPath)) {
                const root = JSON.parse(readFileSync(fulltextPath, 'utf-8'));
                ...
                const md = toMarkdown({
                    ...
                    enforcedAt: law.current.enforcedAt,
                    status: 'in_force',
                }, elements);
                const commitDate = law.current.promulgatedAt ?? law.current.enforcedAt;
