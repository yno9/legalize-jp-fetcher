...
function buildCommitMessage(commitType, title, sourceId, normId) {
    return `[${commitType}] ${title}\n\nSource-Id: ${sourceId}\nSource-Date: ${sourceId.split('_')[1] ?? ''}\nNorm-Id: ${normId}`;
}
function resolveDate(authorDate) {
...
export async function commitRevision(opts) {
...
export async function createFuturePR(repoPath, baseBranch, branchName, filePath, content, message, authorDate, enforcedAt) {
    const git = simpleGit(repoPath);
    const fs = await import('fs/promises');
    const path = await import('path');
    ...
    try {
        execSync(`gh pr create --repo $(git -C ${repoPath} remote get-url origin) --base ${baseBranch} --head ${branchName} --title "${message}" --body "Enforced on: ${enforcedAt}"`, { stdio: 'pipe' });
    }
    catch {
