...
function loadFixture(filename) {
    return JSON.parse(readFileSync(join(FIXTURES, filename), 'utf-8'));
}
function renderFixture(filename, meta) {
...
describe('3. Structure preservation', () => {
    ...
    it('民法: contains Articles', () => {
        ...
        const elements = parseFullText(root);
        const main = elements.find((e) => e.elementType === 'MainProvision');
        const hasArticles = (nodes) => nodes.some((n) => n.elementType === 'Article' || hasArticles(n.children));
        expect(hasArticles(main?.children ?? [])).toBe(true);
    });
    it('民法: Chapters exist (possibly nested under Parts)', () => {
        const root = loadFixture('fulltext_minpo_20260401.json');
        const elements = parseFullText(root);
        const hasChapter = (nodes) => nodes.some((n) => n.elementType === 'Chapter' || hasChapter(n.children));
    ...
    it('民法: article nums are ordered', () => {
        ...
        const elements = parseFullText(root);
        const main = elements.find((e) => e.elementType === 'MainProvision');
        const articles = (main?.children ?? []).flatMap((c) => c.elementType === 'Article' ? [c] : c.children.filter((x) => x.elementType === 'Article'));
